...
def _fill_df_from_corelogic(df: pd.DataFrame, *, delay: float = 0.5, cap: int = 5000) -> tuple[int, int]:
    """Overlay CoreLogic (propertyvalue.co.nz) data onto the pipeline INPUT for
    listings missing a floor or land area — the pricing-critical fields — BEFORE
    the pipeline runs, so the valuation is computed on the filled numbers.

    Only looks up rows that are actually missing a size field (a bounded subset).
    Throttled, circuit-breaks if CoreLogic starts refusing, and degrades to a
    no-op if it's unreachable (the row just prices on what it has, as before).
    ...
    filled = looked = consec_fail = 0
    for idx, row in df.iterrows():
        if looked >= cap:
            break
        # Only spend a lookup when a size field (which drives pricing) is missing.
        ...
        if not pv:
            consec_fail += 1
            if consec_fail >= 40:
                print(f"  [ingest] CoreLogic fill stopped after {consec_fail} misses (likely rate-limited)")
                break
            time.sleep(delay)
            continue
        consec_fail = 0
